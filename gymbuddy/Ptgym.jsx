...
const Ptgym = ({ navigation }) => {
  const [personaltrainergym, setPersonalTrainerGym] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [filteredGyms, setFilteredGyms] = useState([]);
  const ptgyms = firebase.firestore().collection("ptgyms");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setFilteredGyms(personaltrainergymData); // Initialize filtered gyms with all gyms
      } catch (error) {
  ...
  const navigateToDetail = (gymId) => {
    navigation.navigate("GymDetailPage", { gymId });
  };
  ...
  return (
    <SafeAreaView style={styles.ptpage}>
      ...
      <View>
        <Text style={styles.pttext}>PERSONAL TRAINER GYMS</Text>
      </View>
      <TextInput
